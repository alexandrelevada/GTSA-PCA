...
Python script to reproduce the results depicted in Table 3
...
#              Data loading
#############################################
# Agglomerative: Supervised PCA x GTSA-PCA
...
#X = skdata.fetch_openml(name='AP_Endometrium_Prostate', version=1)
...
############ Supervised PCA
dados_spca = SupervisedPCA(dados, target, d)
dados_spca = dados_spca.T
L_spca = Clustering(dados_spca.real, target)
...
# Plot results
if c < 12 and d == 2:
    PlotaDados(dados_spca, L_spca[3], 'Supervised PCA')    
    PlotaDados(dados_gtsapca, L_gtsapca[3], 'GTSA-PCA')

# For printing each row of Table 3
#print(str(L_spca[0])+'\t'+str(L_spca[1])+'\t'+str(L_spca[2])+'\t'+str(L_gtsapca[0])+'\t'+str(L_gtsapca[1])+'\t'+str(L_gtsapca[2]))