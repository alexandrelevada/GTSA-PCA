...
Python script to reproduce the results depicted in Table 7
...
def GTSA_PCA(dados, curvatures, nn, h, d):
    ...
    # Replace the Euclidean distance by the Wasserstein distance
    for u, v, w in G.edges(data=True):
       w['weight'] = wasserstein_distance(dados[u, :], dados[v, :])
...
#              Data loading
#############################################
# HDBSCAN: UMAP x GTSA-PCA-W (Wasserstein distance)
#X = skdata.load_iris()
...
#X = skdata.fetch_openml(name='OVA_Kidney', version=1)
#X = skdata.fetch_openml(name='Breast', version=1)
...
############# UMAP
model = umap.UMAP(n_components=d, random_state=42)
dados_umap = model.fit_transform(dados)
L_umap = Clustering(dados_umap.real, target)
...
# Plot results
if c < 12 and d == 2:
    PlotaDados(dados_umap, L_umap[3], 'UMAP')
    PlotaDados(dados_gtsapca, L_gtsapca[3], 'GTSA-PCA')

# For printing each row of Table 6
#print(str(L_umap[0])+'\t'+str(L_umap[1])+'\t'+str(L_umap[2])+'\t'+str(L_gtsapca[0])+'\t'+str(L_gtsapca[1])+'\t'+str(L_gtsapca[2]))